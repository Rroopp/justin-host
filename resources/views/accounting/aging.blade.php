...
<div class="px-4 py-6 sm:px-0">
    ...
    <div class="bg-white shadow rounded-lg p-4 mb-6" x-data="{ asOfDate: '{{ request('as_of_date', date('Y-m-d')) }}' }">
        <form action="{{ route('accounting.aging-report') }}" method="GET" class="flex items-end gap-4">
            <div>
                <label for="as_of_date" class="block text-sm font-medium text-gray-700">As Of Date</label>
                <input type="date" name="as_of_date" id="as_of_date" x-model="asOfDate" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </div>
