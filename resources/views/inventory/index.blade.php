...
@section('content')
<div x-data="inventoryManager()" class="pb-32">
        <!-- Header -->
        <div class="mb-6 flex justify-between items-center">
            ...
            <div class="flex gap-2">
                ...
                <button @click="exportInventory('csv')" class="bg-indigo-100 text-indigo-700 px-4 py-2 rounded-md hover:bg-indigo-200">
                    Export CSV
                </button>
                <button @click="exportInventory('pdf')" class="bg-rose-100 text-rose-700 px-4 py-2 rounded-md hover:bg-rose-200">
                    Export PDF
                </button>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div x-data="{ open: false }" class="relative" @click.outside="open = false">
                                    <button @click="open = !open" type="button" class="text-gray-600 hover:text-gray-900 focus:outline-none">
                                        <i class="fas fa-ellipsis-v fa-lg px-2 py-1"></i>
                                    </button>
                                    ...
                                    <div x-show="open" 
                                        ...
                                        <div class="py-1" role="none">
                                            <button @click="open = false; editItem(item)" class="text-gray-700 hover:bg-gray-100 hover:text-gray-900 group flex w-full items-center px-4 py-2 text-sm text-left">
                                                <i class="fas fa-edit w-5 mr-2 text-indigo-500"></i> Edit
                                            </button>
                                            <button @click="open = false; openBatchesModal(item)" class="text-gray-700 hover:bg-gray-100 hover:text-gray-900 group flex w-full items-center px-4 py-2 text-sm text-left">
                                                <i class="fas fa-layer-group w-5 mr-2 text-purple-500"></i> Batches
                                            </button>
                                            <button @click="open = false; restockItem(item)" class="text-gray-700 hover:bg-gray-100 hover:text-gray-900 group flex w-full items-center px-4 py-2 text-sm text-left">
                                                <i class="fas fa-truck-loading w-5 mr-2 text-green-500"></i> Restock
                                            </button>
                                            <button @click="open = false; openAdjustModal(item)" class="text-gray-700 hover:bg-gray-100 hover:text-gray-900 group flex w-full items-center px-4 py-2 text-sm text-left">
                                                <i class="fas fa-sliders-h w-5 mr-2 text-amber-500"></i> Adjust
                                            </button>
                                            <a :href="'{{ route('inventory.movements.index') }}?inventory_id=' + item.id" class="text-gray-700 hover:bg-gray-100 hover:text-gray-900 group flex w-full items-center px-4 py-2 text-sm text-left">
                                                <i class="fas fa-history w-5 mr-2 text-blue-500"></i> History
                                            </a>
                                            <button @click="open = false; deleteItem(item)" class="text-red-700 hover:bg-red-50 hover:text-red-900 group flex w-full items-center px-4 py-2 text-sm text-left">
                                                <i class="fas fa-trash w-5 mr-2 text-red-500"></i> Delete
                                            </button>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                ...
                                <div class="col-span-3 mt-2">
                                    <div class="flex items-center space-x-2 bg-indigo-50 p-2 rounded-md border border-indigo-100">
                                        <input id="is_vatable" type="checkbox" x-model="form.is_vatable" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                        <label for="is_vatable" class="text-xs text-gray-900 font-medium">
                                            Vatable Item (Apply 16% VAT)
                                        </label>
                                        <span class="text-xs text-gray-500 ml-auto" x-show="form.is_vatable">
                                            +16% VAT
                                        </span>
                                        <span class="text-xs text-gray-500 ml-auto" x-show="!form.is_vatable">
                                            0% Tax
                        ...
                        <div class="space-y-4">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" x-model="restockForm.quantity" required min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Unit Cost *</label>
                                    <input type="number" step="0.01" x-model="restockForm.unit_cost" required min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                </div>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700">Supplier</label>
                                <select x-model="restockForm.supplier_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <div>
                                ...
                                <select x-model="restockForm.payment_method" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="credit">Bill / Credit (Pay Later)</option>
                                    <option value="cash">Direct Payment (Cash/Bank)</option>
                                </select>
                            </div>

                            <div x-show="restockForm.payment_method === 'cash'">
                                <label class="block text-sm font-medium text-gray-700">Payment Account *</label>
                                <select x-model="restockForm.payment_account_id" :required="restockForm.payment_method === 'cash'" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <div>
                                ...
                                <input type="text" x-model="restockForm.reference" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700">Notes</label>
                                <textarea x-model="restockForm.notes" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"></textarea>
                            </div>

                            <!-- Medical: Batch & Expiry -->
                            <div class="grid grid-cols-2 gap-4 bg-yellow-50 p-3 rounded-md border border-yellow-100">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Batch Number (Optional)</label>
                                    <input type="text" x-model="restockForm.batch_number" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Auto-gen if empty">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">Expiry Date (Optional)</label>
                                    <input type="date" x-model="restockForm.expiry_date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <!-- Batches Modal -->
    <div x-show="showBatchesModal" class="fixed z-50 inset-0 overflow-y-auto" style="display: none;" @keydown.escape.window="showBatchesModal = false">
        <div class="flex items-center justify-center min-h-screen p-4 text-center">
            <div class="fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity" @click="showBatchesModal = false"></div>
            <div class="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:w-full sm:max-w-2xl flex flex-col">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">
                        Batch History: <span x-text="targetBatchItem.product_name"></span>
                    </h3>
                    ...
                    <div class="overflow-x-auto border rounded-md">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                <template x-for="batch in targetBatchItem.batches" :key="batch.id">
                                    <tr :class="{'bg-red-50': isExpired(batch.expiry_date)}">
                                        <td class="px-4 py-2 text-sm font-medium text-gray-900" x-text="batch.batch_number"></td>
                                        <td class="px-4 py-2 text-sm text-gray-500">
                                            <span x-text="batch.expiry_date || 'N/A'"></span>
                                            <span x-show="isExpired(batch.expiry_date)" class="text-xs text-red-600 font-bold ml-1">(Expired)</span>
                                        </td>
                                        <td class="px-4 py-2 text-sm text-gray-900" x-text="batch.quantity"></td>
                                        <td class="px-4 py-2 text-sm text-gray-500" x-text="batch.location_id ? 'Consignment' : 'Main Store'"></td>
                                        <td class="px-4 py-2 text-right text-sm text-gray-500" x-text="formatCurrency(batch.cost_price)"></td>
                                        <td class="px-4 py-2 text-right text-sm">
                                            <a :href="'{{ route('inventory.movements.index') }}?inventory_id=' + targetBatchItem.id + '&batch_id=' + batch.id" 
                                ...
                                <tr x-show="!targetBatchItem.batches || targetBatchItem.batches.length === 0">
                                    <td colspan="5" class="px-4 py-6 text-center text-gray-500">No batches found for this product.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="bg-white border-t border-gray-100 px-6 py-4 flex justify-end">
                    <button @click="showBatchesModal = false" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200">Close</button>
                </div>
            </div>
        </div>
    </div>
...
window.inventoryManager = function() {
    return {
        ...
        restockForm: {
            ...
            unit_cost: 0,
            supplier_id: '',
            payment_method: 'credit', // 'credit' or 'cash'
            payment_account_id: '',
            reference: '',
            reference: '',
            notes: '',
            batch_number: '', // Medical: Batch
            expiry_date: ''   // Medical: Expiry
        },
        showBatchesModal: false,
        targetBatchItem: { batches: [] },
        adjustItem: {},
        adjustmentForm: {
        ...
        async loadAuxiliaryData() {
            // Load vendors and payment accounts for restock modal
            try {
                ...
                this.paymentAccounts = (accountsRes.data.data || accountsRes.data).filter(acc => 
                    ['Cash', 'Bank'].includes(acc.type)
                );
            } catch (error) {
        ...
        async loadAuxiliaryData() {
            // Load Vendors
            try {
                const response = await axios.get('/vendors?format=json');
                this.vendors = response.data.data || response.data;
            } catch (error) {
                console.error('Error loading vendors:', error);
                // Fail silently (user might not have permission)
            }

            // Load Payment Accounts (Assets)
            try {
                const response = await axios.get('/accounting/chart-of-accounts?type=Asset&format=json');
                this.paymentAccounts = response.data.data || response.data;
            } catch (error) {
        ...
        async confirmRestock() {
            try {
                await axios.post(`/inventory/${this.targetRestockItem.id}/restock`, this.restockForm);
                this.showRestockModal = false;
                this.loadInventory();
                window.location.reload(); // Reload for flash message and fresh data
            } catch (error) {
                alert('Restock failed: ' + (error.response?.data?.message || error.response?.data?.error || error.message));
            }
        },

        openBatchesModal(item) {
            this.targetBatchItem = item;
            // Ensure batches array exists (it should be loaded via 'with(batches)')
            if (!this.targetBatchItem.batches) {
                 this.targetBatchItem.batches = [];
            }
