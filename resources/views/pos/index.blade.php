...
<script>
    ...
    window.posSystem = function() {
        ...
        return {
            ...
            locations: [], // Phase 3: Locations
            selectedLocationId: '', // Phase 3: Location Context
            availableLpos: [],
            selectedLpoId: '',
            lpoNumber: '',
            processing: false,
            ...
            // Batch Selection Modal (Traceability)
            showBatchModal: false,
            selectedProductForBatch: null,
            selectedBatchForAdd: null,
            ...
            initData() {
                // Always load initial products (top 30)
                this.searchProducts('');
                
                if (this.customers.length === 0) {
                     this.loadCustomers();
                }
                this.loadLocations(); // Phase 3
                ...
                this.$watch('selectedLocationId', (newId) => {
                    this.recalculateStock();
                });
            },

            // Computed: Detect if selected location is a surgical set
            get isSetTransaction() {
                const loc = this.locations.find(l => l.id == this.selectedLocationId);
                return loc && loc.type === 'set';
            },
            ...
            get caseDetailsRequired() {
                return this.isSetTransaction;
            },
            async loadProducts() {
                // Legacy method support - redirects to search
                this.searchProducts('');
            },
            ...
            async searchProducts(query) {
                // Server-side search
                try {
                    const response = await axios.get('/pos/search', { params: { q: query } });
                    this.products = response.data || [];
                    this.recalculateStock(); // Apply location-based stock logic to the fetching results
            ...
            recalculateStock() {
                // Determine context: Specific Set or Main Store
                const targetLocId = this.selectedLocationId;

                this.products.forEach(p => {
                    // Backup original master stock if not done
                    if (p._original_stock === undefined) {
                        p._original_stock = p.quantity_in_stock;
                    }

                    // Start with legacy stock assumption
                    if (!p.batches || p.batches.length === 0) {
                        // Legacy Item:
                        // If Target is SET -> 0 (Legacy items are assumed Main Store only)
                        // If Target is Main (null) -> Full Original Stock
                        p.quantity_in_stock = targetLocId ? 0 : p._original_stock;
                    } else {
                        // Batched Item: Sum strictly by location
                        const validBatches = p.batches.filter(b => {
                            if (targetLocId) {
                                return b.location_id == targetLocId;
                            } else {
                                // Main Store Context: Include batches with NO location (NULL)
                                return !b.location_id; 
                            }
                        });
                        
                        p.quantity_in_stock = validBatches.reduce((sum, b) => sum + parseFloat(b.quantity), 0);
                    }
                });
                
                // No need to client-side filter products as API did it, but packages still need it
                ...
                try {
                    ...
                    details.items.forEach(item => {
                        this.cart.push({
                            ...
                            // NEW: Track source location for components too
                            location_id: this.selectedLocationId,
                            location_name: this.selectedLocation ? this.selectedLocation.name : ''
                        });
                    });
            ...
            addToCart(product) {
                ...
                // For medical implants requiring serial tracking, force batch selection
                if (product.requires_serial_tracking) {
                    // Filter batches by location context
                    const targetLocId = this.selectedLocationId;
                    const availableBatches = (product.batches || []).filter(b => {
                        if (targetLocId) {
                            return b.location_id == targetLocId && b.quantity > 0;
                        } else {
                            return !b.location_id && b.quantity > 0;
                        }
                    });

                    if (availableBatches.length === 0) {
                        alert(`⚠️ No Batches Available\n\n${product.product_name} requires batch selection, but no batches are available in the current location.\n\nPlease add stock first.`);
                        return;
                    }

                    // Open batch selection modal
                    this.selectedProductForBatch = product;
                    this.selectedBatchForAdd = null;
                    this.showBatchModal = true;
                    return; // Wait for user to select batch
                }
                // -----------------------------
                ...
                if (existingItem) {
                    ...
                    const sourceLocationId = this.selectedLocationId;
                    const sourceLocationName = this.selectedLocation ? this.selectedLocation.name : '';
            ...
            confirmBatchSelection() {
                if (!this.selectedBatchForAdd) {
                    alert('Please select a batch.');
                    return;
                }

                const product = this.selectedProductForBatch;
                const batch = this.selectedBatchForAdd;
                const type = product.is_rentable ? 'rental' : 'sale';
                ...
                this.cart.push({
                    ...
                    location_id: sourceLocationId,
                    location_name: sourceLocationName,
                    batch_number: batch.batch_number // CRITICAL: Attach batch
                });

                // Close modal
                this.showBatchModal = false;
                this.selectedProductForBatch = null;
                this.selectedBatchForAdd = null;
            },
            ...
            async completeSale(status = 'completed') {
                ...
                // SURGICAL SETS: Validate case details
                if (this.isSetTransaction) {
                    if (!this.caseDetails.surgeon_name || !this.caseDetails.patient_name) {
                        alert('⚠️ Surgery Details Required\n\nPlease enter:\n• Patient Name\n• Surgeon Name\n\nThese are required when using a surgical set.');
                        return;
                    }
                }
                ...
                try {
                    const saleData = {
                        sale_type: this.isSetTransaction ? 'surgery_usage' : 'sale',
                        items: this.cart.map(item => ({
                            // Use item.id for packages (since product_id is null)
                            id: item.type === 'package_header' ? item.id : item.product_id,
                            name: item.product_name.replace('📦 ', ''), // Send clean name for receipt
                            quantity: item.quantity,
                            type: item.type,
                            price: item.unit_price,
                            location_id: item.location_id // Pass item location context
                        })),
                        ...
                        location_id: this.selectedLocationId || null, // Phase 3: Location Context
...
<div x-data="posSystem()" x-init="initData()" class="h-[calc(100vh-6rem)] flex flex-col">
    ...
    <div class="flex-none flex items-center justify-between mb-2">
        ...
        <div class="flex gap-2">
            ...
            <!-- Location Selector -->
            <select x-model="selectedLocationId" class="text-xs border-gray-300 rounded py-1 px-2 focus:ring-indigo-500 focus:border-indigo-500">
    ...
    <div class="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-12 gap-4 h-full">
        ...
        <div class="lg:col-span-9 flex flex-col md:flex-row gap-4 h-full min-h-0">
            ...
            <div class="flex-1 flex flex-col bg-white shadow rounded-lg overflow-hidden min-w-0 h-full">
                ...
                <!-- Surgery Mode Indicator -->
                <div x-show="isSetTransaction" x-cloak class="px-3 py-2 bg-blue-50 border-l-4 border-blue-400 flex-none">
                    <div class="flex items-start">
                        ...
                        <div class="flex-1">
                            <p class="text-xs font-semibold text-blue-700">Surgery Mode</p>
                            <p class="text-[10px] text-blue-600 mt-0.5">
                                From: <span class="font-medium" x-text="selectedLocation?.name"></span>
                            </p>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="p-2 pl-3 flex justify-between items-start gap-2">
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="mt-1 flex flex-wrap gap-2 text-[10px] text-gray-500">
                                        ...
                                        <div x-show="item.location_name" class="bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded border border-blue-100 flex items-center">
                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                                            <span x-text="item.location_name"></span>
                                        </div>
                                        
                                        <!-- Package Component Hint -->
                                        <div x-show="item.type === 'package_component'" class="bg-purple-50 text-purple-700 px-1.5 py-0.5 rounded border border-purple-100">
                                            Included in Package (Stock Only)
                                        </div>

                                        <div x-show="item.size"><span class="font-medium text-gray-700">Size:</span> <span x-text="item.size + item.size_unit"></span></div>
                                    ...
                                    <!-- Batch Selection -->
                                    <div class="mt-1" x-show="!item.is_package">
                                        <select x-model="item.batch_number" class="w-full text-[10px] border-gray-200 rounded px-1 py-0.5 focus:ring-indigo-500 focus:border-indigo-500 bg-gray-50">
                                            <option value="">FIFO (Oldest First)</option>
                                            <template x-for="batch in (products.find(p => p.id === item.product_id)?.batches || [])" :key="batch.id">
                                                <option :value="batch.batch_number" x-text="`${batch.batch_number} (Exp: ${batch.expiry_date || '-'}, Qty: ${batch.quantity})`" :disabled="batch.quantity <= 0"></option>
                                            </template>
                                        </select>
                                    </div>
                                </div>
        ...
        <div class="lg:col-span-3 bg-white shadow rounded-lg flex flex-col h-full overflow-hidden border border-gray-200">
            <div class="p-4 flex-1 overflow-y-auto space-y-6">
                ...
                <div>
                    ...
                    <div class="relative" @click.outside="showCustomerResults = false">
                        ...
                        <!-- Customer Dropdown -->
                         <div x-show="showCustomerResults" class="absolute z-30 mt-1 w-full bg-white shadow-xl rounded-md border border-gray-200 max-h-60 overflow-y-auto" style="display: none;">
            ...
            <div class="p-4 bg-gray-50 border-t flex-none">
                ...
                <div class="grid grid-cols-4 gap-2">
                    <button 
                        @click="
                            if(isSetTransaction && (!caseDetails.patient_name || !caseDetails.surgeon_name)) {
                                alert('Surgery details (Patient & Surgeon) are required!');
                                return;
                            }
                            completeSale()
                        "
    ...
    <!-- Batch Selection Modal (Traceability Enforcement) -->
    <div x-show="showBatchModal" x-transition.opacity class="fixed z-50 inset-0 overflow-y-auto" style="display: none;" x-cloak @keydown.escape.window="closeBatchModal()">
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-black/60 backdrop-blur-sm transition-opacity" @click="closeBatchModal()"></div>
            <div class="relative inline-block align-middle bg-white rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div class="bg-white px-6 pt-6 pb-4">
                    <div class="flex items-start justify-between mb-4">
                        <div>
                            <h3 class="text-lg font-bold text-gray-900">Select Batch/Serial Number</h3>
                            <p class="text-sm text-gray-600 mt-1" x-show="selectedProductForBatch" x-text="selectedProductForBatch?.product_name"></p>
                        </div>
                    ...
                    <div class="bg-amber-50 border-l-4 border-amber-400 p-3 mb-4">
                        <div class="flex">
                            ...
                            <div class="text-sm text-amber-800">
                                <p class="font-semibold">Traceability Required</p>
                                <p class="text-xs mt-1">This is a medical device. You must select the specific batch you are selling for regulatory compliance.</p>
                            </div>
                        </div>
                    </div>

                    <div class="space-y-2 max-h-96 overflow-y-auto">
                        <template x-if="selectedProductForBatch">
                            <template x-for="batch in (selectedProductForBatch.batches || []).filter(b => {
                                const targetLocId = selectedLocationId;
                                if (targetLocId) {
                                    return b.location_id == targetLocId && b.quantity > 0;
                                } else {
                                    return !b.location_id && b.quantity > 0;
                                ...
                                <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-indigo-50 transition-colors"
                                       :class="selectedBatchForAdd?.id === batch.id ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200'">
                                    <input type="radio" 
                                           name="batch_selection" 
                                           :value="batch" 
                                           x-model="selectedBatchForAdd"
                                           class="h-4 w-4 text-indigo-600 focus:ring-indigo-500">
                                    <div class="ml-3 flex-1">
                                        <div class="flex justify-between items-start">
                                            <div>
                                                <p class="text-sm font-bold text-gray-900" x-text="batch.batch_number"></p>
                                            ...
                                            <span class="inline-flex items-center px-2 py-1 rounded text-xs font-bold"
                                                  :class="batch.quantity > 10 ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'"
                                                  x-text="`Qty: ${batch.quantity}`">
                                            </span>
                                        </div>
                                    </div>
                                </label>
                ...
                <div class="bg-gray-50 px-6 py-4 flex gap-3">
                    ...
                    <button @click="confirmBatchSelection()" 
                            :disabled="!selectedBatchForAdd"
                            class="flex-1 px-4 py-2 rounded-lg text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed">
                        Add to Cart
                    </button>
                </div>
            </div>
