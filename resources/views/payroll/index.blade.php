                ...
                @forelse($runs as $run)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            ...
                            @if($run->status === 'DRAFT')
                                <button onclick="updatePayrollStatus({{ $run->id }}, 'COMPLETED')" class="text-green-600 hover:text-green-900 mr-3">Finalize & Pay</button>
                                <button onclick="updatePayrollStatus({{ $run->id }}, 'CANCELLED')" class="text-red-600 hover:text-red-900">Cancel</button>
                            @endif
                        </td>
                    </tr>
                @empty
...
<script>
async function updatePayrollStatus(id, newStatus) {
    if (!confirm(`Are you sure you want to mark this payroll run as ${newStatus}?`)) return;
    
    try {
        await axios.put(`/payroll/${id}/status`, { status: newStatus });
