...
class SalesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('export')) {
            // PERFORMANCE FIX: Eager load payments to prevent N+1 query loop
            // Use cursor or chunking for very large datasets if needed, but eager loading is step 1.
            $sales = $query->with(['payments', 'sale_items', 'customer'])->orderBy('created_at', 'desc')->get();
            
            $data = $sales->map(function($sale) {
                // Safe access to payments
    ...
    public function invoices(Request $request)
    {
        $query = PosSale::query()
            ...
            ->where('sale_status', '!=', 'consignment') // Hidden until reconciled
            ->where(function($q) {
                $q->where('payment_method', 'Credit')
                  ->orWhere('is_reconciled', true);
            })
            ->with('customer')
    ...
    {
        // Allow if standard Credit Invoice OR if it's a Reconciled Surgery Invoice
        // We relax the 'Credit' check for reconciled items as they are valid invoices.
        if ($sale->document_type !== 'invoice') {
             // Basic check: Must be an invoice document
             abort(404);
        }
        
        // Secondary check: If not Credit, must be reconciled or explicit invoice
        if ($sale->payment_method !== 'Credit' && !$sale->is_reconciled) {
             abort(404); 
        }

        // BLOCKER: Explicitly deny access if it's still in 'consignment' status (unreconciled)
        // This prevents printing invoices before the reconciliation logic has updated exact quantities.
        if ($sale->sale_status === 'consignment' && !$sale->is_reconciled) {
             abort(403, 'Invoice not generated yet. Please reconcile first.');
        }
