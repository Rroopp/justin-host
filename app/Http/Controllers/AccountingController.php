...
class AccountingController extends Controller
{
    ...
    public function trialBalance(Request $request)
    {
        ...
        foreach ($accounts as $account) {
            ...
            if (abs($net) > 0.001) {
                ...
                $isDebitBalance = $net > 0;
                $displayBalance = abs($net);

                if ($isDebitBalance) {
                    $totalDebits += $displayBalance;
                } else {
                    $totalCredits += $displayBalance;
                }
                ...
                $trialBalance[] = [
                    ...
                    'debit' => $isDebitBalance ? $displayBalance : 0,
                    'credit' => !$isDebitBalance ? $displayBalance : 0,
                ];
            }
        }
    ...
    public function financialStatements(Request $request)
    {
        ...
        return response()->json([
            ...
            'balance_sheet' => [
                ...
                'equity' => $equity + $netIncome, // Retained Earnings / Net Income included in Equity section total
                'total_liabilities_equity' => $liabilities + $equity + $netIncome,
                'details' => [
                    'assets' => $details['Asset'],
                    'liabilities' => $details['Liability'],
    ...
    public function cashFlow(Request $request)
    {
        ...
        // 2. Adjustments for Non-Cash Items (Depreciation, etc.) - Placeholder
        // Currently assuming no non-cash expenses like depreciation are auto-posted yet.

        // 3. Changes in Working Capital (Operating)
        // (This is a simplified approach looking at net movement of Account Types)
        
        // (Increase) in Current Assets (AR, Inventory) -> Uses Cash
        $currentAssetsChange = JournalEntryLine::join('journal_entries', 'journal_entries.id', '=', 'journal_entry_lines.journal_entry_id')
            ->join('chart_of_accounts', 'chart_of_accounts.id', '=', 'journal_entry_lines.account_id')
            ->whereIn('chart_of_accounts.name', ['Accounts Receivable', 'Inventory']) // Simplification: Target specific known Current Assets
            ->where('journal_entries.status', 'POSTED')
            ->whereBetween('journal_entries.entry_date', [$dateFrom, $dateTo])
            ->sum(DB::raw('debit_amount - credit_amount')); // Positive = Increase

        if (abs($currentAssetsChange) > 0) {
            // Increase in Asset = Decrease in Cash
            $adjustment = -$currentAssetsChange; 
            $operatingAdjustments['Changes in Accounts Receivable & Inventory'] = $adjustment;
            $operatingCashFlow += $adjustment;
        }

        // Increase in Current Liabilities (AP, VAT, Payroll) -> Source of Cash
        $currentLiabilitiesChange = JournalEntryLine::join('journal_entries', 'journal_entries.id', '=', 'journal_entry_lines.journal_entry_id')
            ->join('chart_of_accounts', 'chart_of_accounts.id', '=', 'journal_entry_lines.account_id')
            ->where('chart_of_accounts.account_type', 'Liability') 
            // Ideally filter for Current Liabilities only, but assuming all Liabilities are current for this scope or named specific
            ->where('journal_entries.status', 'POSTED')
            ->whereBetween('journal_entries.entry_date', [$dateFrom, $dateTo])
            ->sum(DB::raw('credit_amount - debit_amount')); // Positive = Increase

        if (abs($currentLiabilitiesChange) > 0) {
            // Increase in Liability = Increase in Cash
            $operatingAdjustments['Changes in Accounts Payable & Liabilities'] = $currentLiabilitiesChange;
            $operatingCashFlow += $currentLiabilitiesChange;
        }

        $operatingCashFlow += $netIncome;

        // 4. Investing Activities (Purchase/Sale of Fixed Assets)
        $investingChange = JournalEntryLine::join('journal_entries', 'journal_entries.id', '=', 'journal_entry_lines.journal_entry_id')
            ->join('chart_of_accounts', 'chart_of_accounts.id', '=', 'journal_entry_lines.account_id')
            ->where('chart_of_accounts.name', 'like', '%Equipment%') // Simple heuristic
            ->orWhere('chart_of_accounts.name', 'like', '%Furniture%')
            ->where('journal_entries.status', 'POSTED')
            ->whereBetween('journal_entries.entry_date', [$dateFrom, $dateTo])
            ->sum(DB::raw('debit_amount - credit_amount')); // Positive = Purchase

        $investingCashFlow = -$investingChange; // Purchase is cash out

        // 5. Financing Activities (Equity, Loans)
        $financingChange = JournalEntryLine::join('journal_entries', 'journal_entries.id', '=', 'journal_entry_lines.journal_entry_id')
            ->join('chart_of_accounts', 'chart_of_accounts.id', '=', 'journal_entry_lines.account_id')
            ->where('chart_of_accounts.account_type', 'Equity')
            ->whereRaw("chart_of_accounts.name NOT LIKE '%Retained Earnings%'") // Exclude Retained Earnings
            ->where('journal_entries.status', 'POSTED')
            ->whereBetween('journal_entries.entry_date', [$dateFrom, $dateTo])
            ->sum(DB::raw('credit_amount - debit_amount')); // Positive = Inflow

        $financingCashFlow = $financingChange;

        $netCashChange = $operatingCashFlow + $investingCashFlow + $financingCashFlow;

        return view('accounting.cash-flow', compact(
            'dateFrom', 'dateTo', 
            'netIncome', 'operatingAdjustments', 'operatingCashFlow', 
            'investingCashFlow', 'financingCashFlow', 'netCashChange'
        ));
    }
    ...
    public function indexShareholders(Request $request)
    {
        ...
        $staffList = \App\Models\Staff::select('id', 'first_name', 'last_name', 'username')->get();

        return view('accounting.shareholders.index', compact('shareholders', 'assetAccounts', 'staffList'));
    }
