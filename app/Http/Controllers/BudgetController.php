...
class BudgetController extends Controller
{
    ...
    public function export(Request $request, Budget $budget)
    {
        ...
        $budget->load(['lineItems', 'creator']);

        if ($format === 'csv') {
            $filename = 'budget_' . $budget->reference_number . '_' . date('Y-m-d') . '.csv';
            ...
            return response()->stream(function() use ($budget) {
                $file = fopen('php://output', 'w');
                
                // Header
                fputcsv($file, ['Category', 'Subcategory', 'Description', 'Allocated', 'Spent', 'Remaining', 'Status']);
                ...
                foreach ($budget->lineItems as $item) {
                     fputcsv($file, [
                        ...
                        $item->remaining_amount < 0 ? 'Over Budget' : 'Under Budget'
                    ]);
                }
                
                // Totals
                fputcsv($file, []);
                fputcsv($file, ['TOTALS', '', '', $budget->total_allocated, $budget->total_spent, $budget->total_remaining]);
        ...
        return $pdf->download('budget_' . $budget->reference_number . '.pdf');
    }
