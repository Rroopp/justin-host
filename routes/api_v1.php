...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::post('/accounting/journal-entries/{journalEntry}/cancel', [AccountingController::class, 'cancelJournalEntry'])->middleware('role:admin,accountant'); // NOTE: trial-balance and financial-statements GET routes are served from web.php (HTML + JSON via content negotiation)
    ...
    Route::get('/locations', [\App\Http\Controllers\StockTransferController::class, 'locations'])->middleware('role:admin,staff');
    Route::get('/stock-transfers', [\App\Http\Controllers\StockTransferController::class, 'index'])->middleware('role:admin,staff');
    Route::post('/stock-transfers', [\App\Http\Controllers\StockTransferController::class, 'store'])->middleware('role:admin,staff');
    Route::post('/stock-transfers/{id}/complete', [\App\Http\Controllers\StockTransferController::class, 'complete'])->middleware('role:admin');
});
